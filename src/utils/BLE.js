...
export default function BLE({
    connect = false,
    disconnect = false,
    primaryServiceUuid = parseInt(0xEFF0), sendCharUuid = parseInt(0xEFF1),
    command, vendorId,
    onSuccessReceived,
        ...
        const handleConnect = async () => {
            ...
            if (connect && support) {
                ...
                if (bluetoothDevice != null && bluetoothDevice) {
                    ...
                    if (!sendCharacteristic) {
                        return toast.error('Whoops, failed. Retry: Send characteristic not found.');
                    }
    ...
    return (
        <div className="w-full grid grid-cols-2 gap-2">
            <div className="flex gap-1">
                <button className={`btn-circle btn-xs ${support ? 'btn-info' : 'btn-error'}`} /><label>Support BLE</label>

            </div>
            <div className="flex gap-1">
                <button className={`btn-circle btn-xs ${connected ? 'btn-info' : 'btn-error'}`} /><label> {vendorId}</label>
            </div>
            <div className="flex gap-1 border-t pt-1">
                <button className={`btn-circle btn-xs ${sent ? 'btn-info' : 'btn-error'}`} /><label>Sent</label>
            </div>
            <div className="flex gap-1 border-t pt-1">
                <button className={`btn-circle btn-xs ${received ? 'btn-info' : 'btn-error'}`} /><label>Received</label>
            </div>
        </div>
    )
}